package com.leowong.project.client.media.ijk
...
import com.leowong.project.client.media.IMediaPlayerControl
import com.leowong.project.client.media.IMediaPlayerListener
...
class IjkVideoView : FrameLayout, IMediaPlayer.OnPreparedListener, IMediaPlayer.OnCompletionListener,
    ...
    var mVideoDetail: VideoDetail? = null
    ...
    fun initPlayer() {
        LogUtils.d(TAG, "initPlayer--> " + mVideoDetail?.video_url)
        if (mediaPlayer != null) {
            mediaPlayer?.stop()
            mediaPlayer?.reset()
            mediaPlayer?.release()
    ...
    fun startPlay(videoDetail: VideoDetail) {
...
//        mediaPlayer!!.setDisplay(surfaceView?.holder)
    ...
    fun onResume() {
        LogUtils.d(TAG, "onResume  " + mVideoDetail?.video_url)
        if (isFreeze) {
            isFreeze = false
            if (isPrepared) {
                start()
...
//            start()//暂停状态下会有黑屏情况，改为恢复状态继续播放
    ...
    fun onPause() {
        LogUtils.d(TAG, "onPause  " + mVideoDetail?.video_url)
        if (isPrepared) {
            savePlayerState()
            ...
            LogUtils.d(TAG, "果播放器没有prepare完成")
            isFreeze = true
        }
    ...
    //TODO 播放视频
    fun startPlay(videoDetail: VideoDetail, seekPosition: Long) {
        initPlayer()
        isPrepared = false
        isCompleted = false
        ...
        try {
            val videoPath = videoDetail.video_url
    ...
    override fun onSurfaceCreated(holder: IRenderView.ISurfaceHolder, width: Int, height: Int) {
        LogUtils.d(TAG, "surfaceCreated  " + mVideoDetail?.video_url)
    ...
    override fun onSurfaceDestroyed(holder: IRenderView.ISurfaceHolder) {
        LogUtils.d(TAG, "surfaceDestroyed  " + mVideoDetail?.video_url)
...
//    }
    ...
    override fun play(videoDetail: VideoDetail?, position: Long) {
